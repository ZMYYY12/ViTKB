...
def train_one_epoch(model, optimizer, data_loader, device, epoch, true_num_classes=3):
    model.train()
    ...
    for step, data in enumerate(data_loader):
        ...
        # 关键修改：截取前true_num_classes个类别
        pred = model(images.to(device))[:, :true_num_classes]
...
@torch.no_grad()
def evaluate(model, data_loader, device, epoch, true_num_classes=3):
    loss_function = torch.nn.CrossEntropyLoss()
    model.eval()
